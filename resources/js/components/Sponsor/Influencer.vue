        ...
        <div class="card card-custom">
          <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <div class="card-title">
              <h3 class="card-label">Lista de sponsors</h3>
            </div>
            <button
              @click="
                modificar = false;
...
export default {
  ...
  methods: {
    ...
    async eliminar(sponsor) {
      swal({
        title: "Estas seguro de eliminar la sponsor " + sponsor.nombre + "?",
        ...
        if (willDelete) {
          let url = "/admin/sponsors/" + sponsor.id;
          axios
            .delete(url)
            .then((res) => {
              this.getsponsors();
              swal("Exito!", "sponsor eliminada correctamente", "success");
            })
            .catch((error) => {
              console.log(error);
    ...
    guardar() {
      if (this.modificar) {
        let url = "/admin/sponsors/" + this.id;
        axios
    ...
    abrirModal(data = {}) {
      ...
      if (this.modificar) {
        ...
        this.sponsor.descripcion = data.descripcion;
      } else {
        this.id = 0;
        this.tituloModal = "Crear sponsor";
        this.sponsor.nombre = "";
        this.sponsor.descripcion = "";
      }
    },
    cerrarModal() {
      this.modal = 0;
